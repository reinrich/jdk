...
Address MacroAssembler::spill_address(int size, int offset, Register tmp)
{
  ...
  if ((offset & (size-1)) && offset >= (1<<8)) {
    if ((size == StackAlignmentInBytes) && !is_aligned(offset, StackAlignmentInBytes)) {
      log_develop_trace(newcode)("spill_address: unaligned vector spill slots (size:%d offset:%d)", size, offset);
    }
...
Address MacroAssembler::sve_spill_address(int sve_reg_size_in_bytes, int offset, Register tmp) {
  ...
  if ((sve_reg_size_in_bytes != StackAlignmentInBytes) || is_aligned(offset, StackAlignmentInBytes)) {
    log_develop_trace(newcode)("sve_spill_address: unaligned vector spill slots (size:%d offset:%d)", sve_reg_size_in_bytes, offset);
  }
